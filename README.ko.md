...
crates/
  ...
  mcp-server/               # rinda-mcp (Rust, OAuth 지원 HTTP MCP 서버)
```
...
RINDA MCP 서버는 원격으로 호스팅되며, OAuth를 지원하는 모든 MCP 호환 클라이언트(Claude Desktop, Cursor 등)에서 사용할 수 있습니다. 로컬 설치가 필요 없습니다.

**서버 URL:**

| 환경 | URL |
|------|-----|
| Beta (개발/테스트) | `https://beta-mcp.rinda.ai` |
...
서버는 OAuth 2.0과 동적 클라이언트 등록을 지원합니다. 첫 연결 시 브라우저에서 RINDA를 통한 Google 인증이 진행됩니다. 토큰은 클라이언트가 자동으로 관리합니다.
...
| `/.well-known/oauth-protected-resource` | RFC 9728 보호된 리소스 메타데이터 |
| `/oauth/register` | 동적 클라이언트 등록 (RFC 7591) |
...
| 인증 방식 | RINDA를 통한 Google OAuth 2.0 |
| CLI | Rust (크로스 플랫폼: Linux, macOS, Windows) |
| MCP 서버 | rinda-mcp (OAuth 지원 HTTP 전송, 원격 호스팅) |
| SDK | OpenAPI 스펙에서 progenitor로 자동 생성 |
| 토큰 유효기간 | Access: 1시간 (자동 갱신), Refresh: 14일 |
| 자격증명 | CLI: `~/.rinda/credentials.json`, MCP: OAuth 세션 토큰 |
| 라이선스 | MIT |
